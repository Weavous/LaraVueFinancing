        ...
        <div class="bg-white shadow-md rounded overflow-scroll h-160">
          <table class="min-w-max w-full table-auto">
            ...
            <tbody class="text-gray-600 text-sm font-light">
              <tr v-for="brand in this.brands" :key="brand.id" class="border-b border-gray-200 hover:bg-gray-100">
                ...
                <td class="py-3 px-6 text-left">
                  <div class="flex items-center">
                    <div class="mr-2">
                      <img class="w-6 h-6 rounded-full" v-bind:src="brand.logo">
                    </div>
...
<script>
import brands from "../services/brands.js";

import EyeIcon from "../components/EyeIcon.vue";
import DestroyIcon from "../components/DestroyIcon.vue";
...
export default {
  ...
  mounted() {
    brands.index().then((response) => {
      this.brands = response.data;
    });
  },
  methods: {
    destroy: (id) => {
      brands.destroy(id).then(response => {
